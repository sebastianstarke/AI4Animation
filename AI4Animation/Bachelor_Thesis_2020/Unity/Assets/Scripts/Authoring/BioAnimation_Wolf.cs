...
namespace SIGGRAPH_2018
{
	...
	public class BioAnimation_Wolf : MonoBehaviour
	{
		...
		public float SneakSlider = 0f;
		public float EatSlider = 0f;
		public float HydrateSlider = 0f;
		public bool DrawNetworkSpace = false;
		public bool DrawFirstLayer = false;
		...
		private void PredictTrajectory()
		{
			...
			bool control = turn != 0f || move.magnitude != 0f;
			...
			for (int i = RootPointIndex; i < Trajectory.Points.Length; i++)
			{
				...
				Trajectory.Points[i].Styles[6] = SmoothStep(SneakSlider, 2f, .8f);
				Trajectory.Points[i].Styles[7] = SmoothStep(EatSlider,2f,.8f);
				Trajectory.Points[i].Styles[8] = SmoothStep(HydrateSlider, 2f, .8f);
		...
		private void Animate()
		{
			...
			for (int i = 0; i < PointSamples; i++)
			{
				...
				for (int j = 0; j < Controller.GetNames().Length; j++)
				{
					NN.SetInput(start + i * TrajectoryDimIn + (TrajectoryDimIn - Controller.GetNames().Length) + j, GetSample(i).Styles[j]);
				}
			}
			...
			Vector3 rootMotion = 0 * new Vector3(NN.GetOutput(TrajectoryDimOut * 6 + JointDimOut * Actor.Bones.Length + 0), NN.GetOutput(TrajectoryDimOut * 6 + JointDimOut * Actor.Bones.Length + 1), NN.GetOutput(TrajectoryDimOut * 6 + JointDimOut * Actor.Bones.Length + 2));

			rootMotion /= Framerate;
			Vector3 translation = new Vector3(rootMotion.x, 0f, rootMotion.z);
			//Debug.Log(rootMotion.z);
				...
				using (new EditorGUILayout.VerticalScope("Box"))
				{
					...
					if (Target.Inspect)
					{
						using (new EditorGUILayout.VerticalScope("Box"))
						{
							...
							Target.SneakSlider = EditorGUILayout.Slider("Sneak", Target.SneakSlider, 0f, 1f);
							Target.EatSlider = EditorGUILayout.Slider("Eat", Target.EatSlider, 0f, 1f);
							Target.HydrateSlider = EditorGUILayout.Slider("Hydrate", Target.HydrateSlider, 0f, 1f);
							Target.DrawNetworkSpace = EditorGUILayout.Toggle("DrawNetworkSpace", Target.DrawNetworkSpace);
							Target.DrawFirstLayer = EditorGUILayout.Toggle("DrawFirstLayer", Target.DrawFirstLayer);
							Target.DrawSecondLayer = EditorGUILayout.Toggle("DrawSecondLayer", Target.DrawSecondLayer);
							Target.DrawThirdLayer = EditorGUILayout.Toggle("DrawThirdLayer", Target.DrawThirdLayer);
						}
					}
				}
			}
