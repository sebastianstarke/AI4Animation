...
public class StyleModule : Module {
	...
	public float GetSneakValue(Frame frame)
	{
		for (int i = 0; i < Functions.Length; i++)
		{
			if(Functions[i].Name == "sneak" || Functions[i].Name == "Sneak")
			{
				return Functions[i].GetValue(frame);
			}	
		}
		return 0;
	}

	public float GetEatValue(Frame frame)
	{
		for (int i = 0; i < Functions.Length; i++)
		{
			if (Functions[i].Name == "eat" || Functions[i].Name == "Eat")
			{
				return Functions[i].GetValue(frame);
			}
		}
	...
	public float GetHydrateValue(Frame frame)
	{
		for (int i = 0; i < Functions.Length; i++)
		{
			if (Functions[i].Name == "hydrate" || Functions[i].Name == "Hydrate")
			{
				return Functions[i].GetValue(frame);
			}
		}
