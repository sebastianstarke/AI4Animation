					...
					for(int m=1; m<=(Mirror ? 2 : 1); m++) {
						...
						for(int s=0; s<Editor.GetCurrentFile().Data.Sequences.Length; s++) {
							...
							for(int j=1; j<states.Count-1; j++) {
								...
								Y.Feed(Framerate * delta.GetPosition().x, Data.ID.Standard, "RootTranslationX");
								Y.Feed(Framerate * Vector3.SignedAngle(Vector3.forward, delta.GetForward(), Vector3.up), Data.ID.Standard, "RootRotationY");
								Y.Feed(Framerate*delta.GetPosition().z, Data.ID.Standard, "RootTranslationZ");
								Y.Store();
								//
