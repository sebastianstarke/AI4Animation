...
public class ActorAnimation : MonoBehaviour {
	...
	void Awake() {
		...
		//
		PastPositions = new List<Vector3>[Actor.Bones.Length];
		PastForwards = new List<Vector3>[Actor.Bones.Length];
		PastUps = new List<Vector3>[Actor.Bones.Length];
		PastVelocities = new List<Vector3>[Actor.Bones.Length];
		for(int i=0; i<Actor.Bones.Length; i++) {
		...
		//
		//
		FuturePositions = new List<Vector3>[Actor.Bones.Length];
		FutureForwards = new List<Vector3>[Actor.Bones.Length];
		FutureUps = new List<Vector3>[Actor.Bones.Length];
		FutureVelocities = new List<Vector3>[Actor.Bones.Length];
		for(int i=0; i<Actor.Bones.Length; i++) {
	...
	private void Animate() {
		...
		*/
		...
		Vector3 translationalOffset = new Vector3(NN.Model.GetOutput(TrajectoryDimOut*6 + JointDimOut*Actor.Bones.Length + 0), 0f, NN.Model.GetOutput(TrajectoryDimOut*6 + JointDimOut*Actor.Bones.Length + 1));
		float rotationalOffset = NN.Model.GetOutput(TrajectoryDimOut*6 + JointDimOut*Actor.Bones.Length + 2);
		...
		if(Application.isPlaying) {
			...
			for(int i=0; i<6; i++) {
				for(int j=0; j<Actor.Bones.Length; j++) {
					...
					Actor.Bones[j].Transform.position = mat.GetPosition();
					Actor.Bones[j].Transform.rotation = mat.GetRotation();
				}
				Actor.DrawSimple(Color.Lerp(UltiDraw.Blue, UltiDraw.Cyan, 1f - (float)(i+1)/6f).Transparent(0.75f));
			}
			*/
			/*
			for(int i=0; i<5; i++) {
	...
	public class ActorAnimation_Editor : Editor {

		public ActorAnimation Target;

		void Awake() {
			Target = (ActorAnimation)target;
		}
