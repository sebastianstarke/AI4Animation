...
public class B_Trajectory : MonoBehaviour {
	...
	public void Target(float rotate, Vector3 move) {
		TargetDirection = Quaternion.AngleAxis(10f*rotate, Vector3.up) * TargetDirection;
		TargetVelocity = Vector3.Lerp(TargetVelocity, Quaternion.LookRotation(TargetDirection, Vector3.up) * (3f * move) / 100f, 1f-TargetSmoothing);
	}
	...
	void OnDrawGizmos() {
		...
		for(int i=0; i<Points.Length; i++) {
			Gizmos.DrawSphere(Points[i].Position, 0.015f);
		}
