...
public class BioAnimation_Old_V : MonoBehaviour {
	...
	void Update() {
		...
		if(MFNN.Parameters != null) {
			...
			for(int i=0; i<Joints.Length; i++) {
				...
				//Debug.Log(Joints[i].name + " " + (start + i*JointDimIn + 9));
	...
	#if UNITY_EDITOR
	[CustomEditor(typeof(BioAnimation_Old_V))]
	public class BioAnimation_Old_V_Editor : Editor {

		public BioAnimation_Old_V Target;

		void Awake() {
			Target = (BioAnimation_Old_V)target;
		}
