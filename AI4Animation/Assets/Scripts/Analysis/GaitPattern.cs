...
public class GaitPattern : MonoBehaviour {
	...
	void OnRenderObject() {
		...
		UltiDraw.DrawGUIRectangle(Rect.GetPosition(), new Vector2(width, height), UltiDraw.DarkGrey, 0.5f*border, UltiDraw.BlackGrey);
		float pivot = 0.5f * height;
		for(int i=1; i<=Feet.Length; i++) {
			pivot -= Rect.H/2f;
			UltiDraw.DrawGUIRectangle(Rect.GetPosition() + new Vector2(0f, pivot), new Vector2(Rect.W, Rect.H), UltiDraw.White);
