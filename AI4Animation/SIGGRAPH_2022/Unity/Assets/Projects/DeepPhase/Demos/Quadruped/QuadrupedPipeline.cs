...
namespace DeepPhase {
    public class QuadrupedPipeline : AssetPipelineSetup {
        ...
        private void ProcessAssets(MotionAsset asset) {
            ...
            //     module.Clear();
            //     module.AddSensor("Hips", Vector3.zero, Vector3.zero, 0.2f*Vector3.one, 1f, LayerMask.GetMask("Ground"), ContactModule.ColliderType.Sphere);
            //     module.AddSensor("Neck", Vector3.zero, Vector3.zero, 0.25f*Vector3.one, 1f, LayerMask.GetMask("Ground"), ContactModule.ColliderType.Sphere);
            //     module.AddSensor("LeftHandSite", Vector3.zero, Vector3.zero, 1f/30f*Vector3.one, 1f, LayerMask.GetMask("Ground"), ContactModule.ColliderType.Sphere);
            //     module.AddSensor("RightHandSite", Vector3.zero, Vector3.zero, 1f/30f*Vector3.one, 1f, LayerMask.GetMask("Ground"), ContactModule.ColliderType.Sphere);
            //     module.AddSensor("LeftFootSite", Vector3.zero, Vector3.zero, 1f/30f*Vector3.one, 1f, LayerMask.GetMask("Ground"), ContactModule.ColliderType.Sphere);
            //     module.AddSensor("RightFootSite", Vector3.zero, Vector3.zero, 1f/30f*Vector3.one, 1f, LayerMask.GetMask("Ground"), ContactModule.ColliderType.Sphere);
            ...
            //     StyleModule.StyleFunction idle = module.AddStyle("Idle");
            //     StyleModule.StyleFunction move = module.AddStyle("Move");
            ...
            //         float movement = root.GetTranslationalSpeed(asset.Frames[f].Timestamp, false);
            //         idle.Values[f] = motion < threshold ? 1f : 0f;
            //         move.Values[f] = 1f - idle.Values[f];
            //         speed.Values[f] = movement;
            //         weights[f] = Mathf.Sqrt(Mathf.Clamp(motion, 0f, threshold).Normalize(0f, threshold, 0f, 1f));
            //     }
            //     {
            //         float[] copy = idle.Values.Copy();
            //         for(int i=0; i<copy.Length; i++) {
            //             idle.Values[i] = copy.GatherByWindow(i, Mathf.RoundToInt(weights[i] * 0.5f*root.Window*asset.Framerate)).Gaussian().SmoothStep(2f, 0.5f);
            //         }
            //     }
            //     {
            //         float[] copy = move.Values.Copy();
            //         for(int i=0; i<copy.Length; i++) {
            //             move.Values[i] = copy.GatherByWindow(i, Mathf.RoundToInt(weights[i] * 0.5f*root.Window*asset.Framerate)).Gaussian().SmoothStep(2f, 0.5f);
            //         }
            //     }
            //     {
            ...
            //             speed.Values[i] = copy.GatherByWindow(i, padding).Gaussian(power);
            //             speed.Values[i] = Mathf.Lerp(speed.Values[i], 0f, idle.Values[i]);
            //         }
            //     }
            // }
