...
namespace Eigen {
    public class Matrix {
        ...
        public static Matrix FromBinary(int rows, int cols, string id, string binary) {
            float[] buffer = ReadBinary(binary, rows*cols);
            if(buffer != null) {
            ...
            if(File.Exists(fn)) {
                ...
                for(int i=0; i<size; i++) {
                    try {
                        ...
                        Debug.Log("There was an error reading file at path " + fn + ":");
                        Debug.Log(e);
                        reader.Close();
                        return null;
                    }
        ...
        public static Matrix Normalize(Matrix IN, Matrix mean, Matrix std, Matrix OUT) {
            if(IN.GetRows() != mean.GetRows() || IN.GetRows() != std.GetRows() || IN.GetCols() != mean.GetCols() || IN.GetCols() != std.GetCols()) {
                Debug.Log("Incompatible dimensions for normalization.");
                return IN;
            } else {
        ...
        public static Matrix Renormalize(Matrix IN, Matrix mean, Matrix std, Matrix OUT) {
            if(IN.GetRows() != mean.GetRows() || IN.GetRows() != std.GetRows() || IN.GetCols() != mean.GetCols() || IN.GetCols() != std.GetCols()) {
                Debug.Log("Incompatible dimensions for renormalization.");
                return IN;
            } else {
        ...
        public static Matrix Blend(Matrix M, Matrix W, float w) {
        ...
        public static Matrix BlendAll(Matrix M, Matrix[] W, float[] w, int length) {
