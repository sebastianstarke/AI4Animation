...
public class Transparency : MonoBehaviour {
    ...
    public void FadeIn(float duration, float delay) {
        StartCoroutine(ProcessFade(duration, delay, 1f, 0f));
    }

    public void FadeOut(float duration, float delay) {
        StartCoroutine(ProcessFade(duration, delay, 0f, 1f));
    }

    private IEnumerator ProcessFade(float duration, float delay, float start, float end) {
        if(delay > 0f) {
