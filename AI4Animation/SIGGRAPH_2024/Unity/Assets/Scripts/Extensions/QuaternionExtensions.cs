...
public static class QuaternionExtensions {
	...
	public static Quaternion[] RotationsFrom(this Quaternion[] rotations, Matrix4x4 from, bool inplace) {
		Quaternion[] result = inplace ? rotations : new Quaternion[rotations.Length];
		for(int i=0; i<result.Length; i++) {
			result[i] = rotations[i].RotationFrom(from);
		}
		return result;
	}

	public static Quaternion[] RotationsTo(this Quaternion[] rotations, Matrix4x4 to, bool inplace) {
		Quaternion[] result = inplace ? rotations : new Quaternion[rotations.Length];
		for(int i=0; i<result.Length; i++) {
			result[i] = rotations[i].RotationTo(to);
		}
		return result;
	}
