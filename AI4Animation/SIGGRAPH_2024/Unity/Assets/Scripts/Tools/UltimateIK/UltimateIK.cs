...
namespace UltimateIK {
    ...
    public class IK {
        ...
        public void DeactivateMissingJoints(string[] names) {
            foreach(Joint joint in Joints) {
                joint.Active = names.Contains(joint.Transform.name);
            }
        }
        ...
        public void SetTargets(params Transform[] targets) {
            ...
            for(int i=0; i<Objectives.Length; i++) {
                Objectives[i].TargetPosition = targets[i].position;
                Objectives[i].TargetRotation = targets[i].rotation;
            }
        }
        ...
        public void Solve() {
            ...
            if(IsSetup()) {
                ...
                recursion = new Action<Joint, Joint>((parent, joint) => {
                    ...
                    joint.Length = parent == null ? 1 : (joint.Active && parent.Active ? (parent.Length + joint.Transform.localPosition.magnitude) : parent.Length);
                    ...
                    if(!IsConverged()) {
                        if(AllowRootUpdateX || AllowRootUpdateY || AllowRootUpdateZ) {
                            ...
                            foreach(Objective o in Objectives) {
                                float weight = o.Weight * GetWeight(Joints[o.Joint], o);
                                if(o.Active) {
                                    delta += weight * (o.TargetPosition - Joints[o.Joint].Transform.position);
                                    sum += weight;
                                }
                            }
                ...
                if(joint.Active) {
                    ...
                    foreach(int index in joint.Objectives) {
                        ...
                        if(o.Active && o.SolveRotation) {
                            float weight = o.Weight * GetWeight(joint, o);
                    ...
                    if(sum > 0f) {
                        forward /= sum;
                        up /= sum;
                        joint.Transform.rotation = Quaternion.LookRotation(forward.normalized, up.normalized);
                        joint.ResolveLimits();
                    }
                }
        ...
        public void Draw() {
            ...
            for(int i=1; i<Joints.Length; i++) {
                UltiDraw.DrawLine(Joints[i-1].Transform.position, Joints[i].Transform.position, UltiDraw.Green);
            }
            for(int i=0; i<Joints.Length; i++) {
                UltiDraw.DrawSphere(Joints[i].Transform.position, Quaternion.identity, 0.025f, Joints[i].Active ? UltiDraw.Green : UltiDraw.Red);
            }
            foreach(Objective o in Objectives) {
                UltiDraw.DrawSphere(o.TargetPosition, Quaternion.identity, 0.1f, UltiDraw.Purple.Opacity(0.5f));
            }
            UltiDraw.End();
        }
    }
        ...
        public void ResolveLimits() {
            switch(Type) {
                ...
                case TYPE.HingeX:
                {
                    ...
                    Transform.localRotation = ZeroRotation * Quaternion.AngleAxis(Mathf.Clamp(angle, LowerLimit, UpperLimit), Vector3.right);
                }
                break;
                ...
                case TYPE.HingeY:
                {
                    ...
                    Transform.localRotation = ZeroRotation * Quaternion.AngleAxis(Mathf.Clamp(angle, LowerLimit, UpperLimit), Vector3.up);
                }
                break;
                ...
                case TYPE.HingeZ:
                {
                    ...
                    Transform.localRotation = ZeroRotation * Quaternion.AngleAxis(Mathf.Clamp(angle, LowerLimit, UpperLimit), Vector3.forward);
                }
                break;
